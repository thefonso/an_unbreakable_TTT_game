  ...
  module Minimax 
    ...
    def score_the_boards(board, player)
      ...
      test_boards_hash.each do |key, value|
        if win?(board,player_symbol) != true 
          if win?(board,player_symbol) == true
            scores_hash[value] = (1000 - ply)
          elsif win?(board,player_symbol) == true
            scores_hash[value] = (-1000 + ply)
          else
    ...
    def generate_boards(board, player)
      ...
      new_board_array = []
      #empty_spaces = virtual_board.grid.select{ |v| v == "+" }
      empty_spaces = []
      ...
      empty_spaces.each do |space|
        ...
        if player == "O"
          ...
          new_player = "X"
          @virtual_board_array[@i] = new_board.grid
        else
